...
class CadastrarPaciente():
    ...
    def enviardados(self):
        ...
        nome=self.cxaNome.get() 
        cpf = self.cxaCPF.get() 
        rg = self.cxaRG.get() 
        data_nasc = self.cxaData.get() 
        sexo = self.varSexo.get()
        cep = self.cxaCEP.get() 
        endereco = self.cxaEndereco.get() 
        bairro = self.cxaBairro.get() 
        cidade = self.cxaCidade.get() 
        uf = self.cxaUF.get() 
        celular = self.cxaCelular.get() 
        telefone = self.cxaFixo.get() 
        ...
        query = '''
        INSERT INTO pacientes (nome, cpf, rg, data_nasc, sexo, cep, endereco, bairro, cidade, uf, celular, telefone)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
        '''
        values = (nome, cpf, rg, data_nasc, sexo, cep, endereco, bairro, cidade,uf, celular, telefone)
