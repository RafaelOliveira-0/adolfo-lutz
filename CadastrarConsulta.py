...
class CadastrarConsulta():
    def __init__ (self):
        ...
        self.btnConcluido = CTkButton(self.frmCadastrar, text="Concluído", text_color="#FFFFFF", fg_color="#659BDB", command= lambda: self.enviardados(),
                                      font=("Roboto", 22, "bold"), corner_radius=15, anchor="w", hover_color="#4379B9")#,command= lambda: self.TelaMarcado()) comentado pois a tela estava indisponivel quando fui fazer a integração dessa tela com o banco
    ...
    def enviardados(self):
         fisio = self.cxaFisioterapeuta.get().strip()
         paciente = self.cxaPaciente.get().strip()
         procedimento = self.cxaProcedimento.get().strip()
         data_marcada = self.cxaData.get().strip()
         horario = self.cxaHorario.get().strip()
         if data_marcada:
                 try:
         ...
         query = '''
      INSERT INTO consultas (cod_funcionario, cod_paciente, nome_tratamento, data_consulta, horario) 
      VALUES (%s,%s,%s,%s,%s)
         '''
         values = (fisio, paciente, procedimento, data_marcada,horario)
